/* Copyright (c) 2012 ASE. All Rights Reserved.
 *
...
#ifndef TWI_MASTER_DEV_H
#define TWI_MASTER_DEV_H
...
/*
 * Structure for TWI peripheral in nRF51822:
 *	member:
 *		NRF_TWI_Type* twi_dev:	choose NRF_TWI0 or NRF_TWI1 hardware peripheral
 *		uint8_t sda_pin:	select the SDA pin on nRF51822
 *		uint8_t Scl_pin:	select the SCL pin on nRF51822
 */
typedef struct{
	NRF_TWI_Type* twi_dev;	
	uint8_t sda_pin;
	uint8_t scl_pin;
}TWI_struct;
 ...
 */
bool twi_master_init(TWI_struct *twi_config);
...
bool twi_master_transfer(TWI_struct* twi_config, uint8_t address, uint8_t *data, uint8_t data_length, bool issue_stop_condition);
